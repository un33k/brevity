...
class ArticleFeaturedView(ArticleSearchView):
    """
    ...
    def get_data(self):
        data = super(ArticleFeaturedView, self).get_data()
        data['referrer'] = 'featured'
        return data

    def get_articles_by_search_params(self):
        """
        Returns list of related article based on featured, categories, targets, tags and author.
        """
        cache_on = not is_user_active(self.request, self.article_flavor)
        if cache_on:
            cache_key = self.get_cache_key_for_params_by_type('articles:featured')
